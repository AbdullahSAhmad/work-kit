...
import { AUTO_BLOCK_END, AUTO_BLOCK_START, KNOWLEDGE_DIR } from "../utils/knowledge.js";
...
describe("learnCommand", () => {
  ...
  it("appends a lesson and creates auto-block markers", () => {
    ...
    assert.equal(r.file, "findings.md");

    const findingsPath = path.join(tmp, KNOWLEDGE_DIR, "findings.md");
  ...
  it("routes project types to findings.md and workflow to workflow.md", () => {
    const tmp = makeTmpDir();
    tmpDirs.push(tmp);
    setupSession(tmp);
    ...
    learnCommand({ type: "decision", text: "D", worktreeRoot: tmp });
    learnCommand({ type: "workflow", text: "W", worktreeRoot: tmp });

    const dir = path.join(tmp, KNOWLEDGE_DIR);
    const findings = fs.readFileSync(path.join(dir, "findings.md"), "utf-8");
    assert.ok(findings.includes("L"));
    assert.ok(findings.includes("C"));
    assert.ok(findings.includes("R"));
    assert.ok(findings.includes("D"));
  ...
  it("redacts secrets in text before writing", () => {
    ...
    const content = fs.readFileSync(path.join(tmp, KNOWLEDGE_DIR, "findings.md"), "utf-8");
  ...
  it("extracts typed bullets from state.md ## Observations", () => {
    ...
    // 3 project bullets → findings.md, 1 workflow bullet → workflow.md
    assert.equal(r.byFile.findings, 3);
    assert.equal(r.byFile.workflow, 1, "workflow:test/e2e bullet should be routed (regression: digit in step name)");
    ...
    const findingsMd = fs.readFileSync(path.join(tmp, KNOWLEDGE_DIR, "findings.md"), "utf-8");
    assert.ok(findingsMd.includes("foo.ts is fragile"));
    assert.ok(findingsMd.includes("All inputs validated via Zod"));
    assert.ok(findingsMd.includes("We discovered something useful"));
  });
  ...
  it("ignores bullets under ## Decisions and ## Deviations", () => {
    ...
    assert.equal(r.byFile.findings, 0);
    assert.equal(r.byFile.workflow, 0);
  });
  ...
  it("extract is idempotent (re-run produces only duplicates)", () => {
    ...
    const stateMdPath = path.join(tmp, ".work-kit", "state.md");
    const original = fs.readFileSync(stateMdPath, "utf-8");
    fs.writeFileSync(stateMdPath, original.replace("\n## Deviations", "\n- [risk] one\n- [risk] two\n\n## Deviations"));
