...
import { Location, LoopbackRecord, PhaseName, StepOutcome } from "../state/schema.js";
...
export function countLoopbacksForRoute(loopbacks: LoopbackRecord[], from: Location, to: Location): number {
  return loopbacks.filter(
    (lb) =>
      lb.from.phase === from.phase && lb.from.step === from.step && lb.to.phase === to.phase && lb.to.step === to.step,
  ).length;
}
 ...
 */
export function checkLoopback(phase: PhaseName, step: string, outcome?: StepOutcome): LoopbackResult | null {
  if (!outcome) return null;

  const route = LOOPBACK_ROUTES.find(
    (r) => r.from.phase === phase && r.from.step === step && r.triggerOutcome === outcome,
  );
